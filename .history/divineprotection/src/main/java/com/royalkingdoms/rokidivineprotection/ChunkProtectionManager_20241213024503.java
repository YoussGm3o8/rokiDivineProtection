    ...
    public void onBlockPlace(BlockPlaceEvent event) {
        ...
        if (protectedChunk != null) {
            if (!protectedChunk.getOwner().equals(player.getUniqueId())) {
                ...
                if (protectedChunk.getChestCount() > MAX_CHESTS) {
                    protectedChunk.setProtectionActive(false);
                    player.sendTitle("§cDivine Protection", "§4Protection Disabled: Too Many Chests!", 10, 70, 20);
                }
                saveProtectedChunks();
            }
        }
    ...
    public void onBlockBreak(BlockBreakEvent event) {
        ...
        if (protectedChunk != null) {
            if (!protectedChunk.getOwner().equals(player.getUniqueId())) {
                ...
                if (protectedChunk.getChestCount() <= MAX_CHESTS && !protectedChunk.isProtectionActive()) {
                    protectedChunk.setProtectionActive(true);
                    player.sendTitle("§aDivine Protection", "§2Protection Reactivated!", 10, 70, 20);
                }
                saveProtectedChunks();
            }
        }
